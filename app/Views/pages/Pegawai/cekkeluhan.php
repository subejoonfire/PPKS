                            ...
                            <div class="accordion" id="accordionExample<?php echo $row->id_pengaduan; ?>">
                                <div class="accordion-item">
                                    ...
                                    <div id="collapseOne<?php echo $row->id_pengaduan; ?>" class="accordion-collapse collapse" aria-labelledby="headingOne<?php echo $row->id_pengaduan; ?>" data-bs-parent="#accordionExample<?php echo $row->id_pengaduan; ?>">
                                        ...
                                        <p>Status Pengaduan: <?php echo !empty($row->statuspengaduan) ? $row->statuspengaduan : 'Belum diisi'; ?></p>
                                    </div>
                                </div>
                            </div>
                        </td>
                        <td>
                        <?php if ($row->statuskonsultasi == 'Belum Konsultasi') { ?>
                        <form action="/pegawaikeluhanterima" method="post">
                            <input type="text" value="<?= $row->id_pengaduan ?>" name="id_pengaduan" style="display:none">
                            <button type="submit" class="btn btn-primary" style="margin-top: 5px;">Terima</button>
                        </form>
                        <form action="/pegawaikeluhantolak" method="post">
                            <input type="text" value="<?= $row->id_pengaduan ?>" name="id_pengaduan" style="display:none">
                            <button type="submit" class="btn btn-danger" style="margin-top: 5px;">Tolak</button>
                    <?php } elseif ($row->statuskonsultasi == 'Selesai Konsultasi') { ?>
                        <span class="btn btn-success" style="margin-top: 5px;">Selesai Konsultasi</span>
                    <?php } else { ?>
                        <form action="/pegawaikeluhanselesai" method="post">
                            <input type="text" value="<?= $row->id_pengaduan ?>" name="id_pengaduan" style="display:none">
                            <button type="submit" class="btn btn-success" style="margin-top: 5px;">Selesai</button>
                        </form>
                    <?php } ?>
                                <!-- <br> -->
                        </td>
